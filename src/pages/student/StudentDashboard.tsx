...
import { timetableApi, type TimetableSlot, type DayTimetable } from "../../services/apiClient";
import { StudentTimetableModal } from "../../components/student/StudentTimetableModal";
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    timetableApi.getWeeklyTimetable().then((res) => {
      if (res.success && res.data?.days) {
        const found = res.data.days.find((d: DayTimetable) => d.day === today);
        if (found) {
          setTodaySlots(found.slots);
        } else {
          setTodaySlots(res.data.days[0]?.slots || []);
        }
      }
    });
  }, []);
  ...
  return (
    <StudentLayout>
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium text-sm sm:text-base mt-1">
              Here is your comprehensive academic performance, class schedule, and milestone tracker.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-bold shadow-md shadow-purple-500/20 transition-all flex items-center gap-1.5 cursor-pointer"
            >
        ...
        {/* Today's Schedule & Live Class Card */}
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/60 space-y-5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-extrabold uppercase tracking-wider text-purple-600">
                  Today's Lecture & Lab Schedule ({todayDayName})
                </span>
              </div>
              <h3 className="text-xl font-bold text-slate-900 tracking-tight mt-0.5">
                Classroom Academic Roadmap
              </h3>
            </div>

            <button
              onClick={() => setIsTimetableOpen(true)}
              className="px-4 py-2 rounded-xl bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs font-bold border border-purple-200 transition-colors flex items-center gap-1.5 self-start sm:self-auto cursor-pointer"
            >
              <Calendar className="w-3.5 h-3.5" />
          ...
          {todaySlots.length === 0 ? (
            <p className="text-xs text-slate-400 py-4 text-center">No periods scheduled for today.</p>
          ) : (
                  ...
                  <div
                    ...
                    className={`p-4 rounded-2xl border transition-all flex flex-col justify-between ${
                      isBreak
                        ...
                        : isLab
                        ? "bg-purple-50/60 border-purple-200/60 text-slate-900"
                        : "bg-slate-50/60 border-slate-200/60 text-slate-900"
                    }`}
                  >
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className="px-2 py-0.5 rounded-md bg-white text-slate-700 text-[10px] font-mono font-bold border border-slate-200">
                    ...
                    <div className="pt-2 mt-2 border-t border-slate-200/40 flex items-center justify-between text-[11px] text-slate-500 font-semibold">
                      {slot.room ? (
